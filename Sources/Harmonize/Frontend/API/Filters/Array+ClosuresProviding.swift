...
public extension Array where Element: Declaration & ClosuresProviding {
    ...
    func withClosures(_ predicate: (Closure) -> Bool) -> [Element] {
        with(\.closures) { $0.contains(where: predicate) }
    }

    /// Retrieves a flat array of all closures from the elements in the array.
    ///
    /// - returns: An array of `Closure` instances extracted from each element in the array.

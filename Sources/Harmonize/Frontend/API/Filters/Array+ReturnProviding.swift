...
public extension Array where Element: Declaration & ReturnProviding {
    ...
    func withReturnType(_ predicate: (TypeAnnotation) -> Bool) -> [Element] {
        with(\.returnType) {
    ...
    func withReturnType(named: String) -> [Element] {
        withReturnType { $0.annotation == named }
    }
    ...
    func withReturnTypeEndingWith(_ suffixes: String...) -> [Element] {
        withReturnType {
    ...
    func withReturnType<T>(_ type: T.Type) -> [Element] {
        withReturnType(named: String(describing: type))
    }

    /// Returns an array of elements that do not have a type annotation.
    ///
    /// - Returns: An array of elements with no type annotation.
    func withInferredReturnType() -> [Element] {
        filter { $0.returnType == nil }
    }
}
